...
io.on('connection', function(socket)
{
		...
		socket.on("UpdatePlayerPosition",function(playerID, moveType)
		{
			console.log("HERE!");
			console.log("Player with PlayerID" + playerID + " has changed the position");
			let index = findThePlayer(playerID);
			
			socket.broadcast.emit("FromServer_PlayerMoved", playerID, moveType);
		});
		...
		socket.on("refreshPlayerPosition",function(player_x, player_y, clientID)
		{
			let index = findThePlayer(clientID);
